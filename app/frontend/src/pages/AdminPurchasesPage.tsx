...
export default function AdminReorderPage() {
  ...
  const [sort, setSort] = useState<"best_seller" | "low_stock">("best_seller");
  ...
  const { data: suggestions = [], isLoading: loadingSuggestions } = useQuery({
    queryKey: ["reorder-suggestions", sort],
    queryFn: () => getReorderSuggestions(token!, 15, sort),
    enabled: !!token,
  });
          ...
          <div className="xl:col-span-2">
            <div className="bg-white rounded-2xl border border-[var(--color-border)] shadow-sm overflow-hidden">
              ...
              <div className="p-4 border-b border-[var(--color-border)] flex gap-2">
                <button
                  onClick={() => { setSort("best_seller"); clearCart(); }}
                ...
                <button
                  onClick={() => { setSort("low_stock"); clearCart(); }}
                  className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-colors ${
                    sort === "low_stock"
                      ? "bg-[var(--color-primary)] text-white"
                      : "bg-gray-100 text-gray-500 hover:bg-gray-200"
              ...
              <div className="p-5 border-b border-[var(--color-border)] flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-[var(--color-text)]">
                    {sort === "best_seller" ? "🏆 많이 팔린 상품" : "📉 재고 부족 상품"}
                  </h2>
                  <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">
                    {sort === "best_seller"
                      ? "판매량 기준 상위 상품 · 추천 수량은 최근 판매량의 50%"
                      : "재고가 적은 순으로 정렬 · 빠른 발주가 필요한 상품"}
                  </p>
                </div>
                  ...
                  {suggestions.map((s, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            <span className="text-xs text-[var(--color-text-secondary)]">
                              총 판매 {s.total_sold.toLocaleString()}개
                            </span>
                            {/* 재고 뱃지 */}
                        ...
                        {!selected && (
                          <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-500 flex-shrink-0">
                            추천 {s.suggested_quantity}개
                          </span>
                        )}
                        ...
                        <button
                          ...
                          className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors flex-shrink-0 ${
                            selected ? "bg-[var(--color-primary)] text-white" : "bg-gray-100 text-gray-400 hover:bg-gray-200"
                          }`}
                        >
                          {selected ? "✓" : "+"}
                        </button>
