...
> **`git clone` 후 스크립트 2개만 실행하면 됩니다.**

#### 🍎 macOS / 🐧 Linux

```bash
# Step 1: DB(Docker MySQL) + venv + 시드 데이터 세팅 (최초 1회)
cd app
./setup_all.sh
...
```cmd
REM Step 1: DB(Docker MySQL) + venv + 시드 데이터 세팅 (최초 1회)
cd app
setup_all.bat
...
> **`db/seeds/full_seed_latest.sql.gz`** 가 없으면 Step 1/3에서 시드 import가 스킵됩니다.
> 시드 파일이 있는 팀원에게 받아서 `db/seeds/` 폴더에 위치시키세요.

---

### 공용 EC2 DB 사용 (팀 데이터 공유)

개인 로컬 Docker MySQL 대신 팀 공용 EC2 MySQL을 사용하면 모든 팀원이 동일한 데이터를 공유할 수 있습니다.
...
**방법**: `.env`의 `DATABASE_URL`을 EC2 주소로 변경 후 `--skip-db` 플래그 사용

```bash
# .env 수정
DATABASE_URL=mysql+pymysql://ebrcs_app:<pw>@<EC2_IP>:3306/mydb

# setup_all 실행 (Docker MySQL 기동 건너뜀)
cd app
./setup_all.sh --skip-db   # macOS/Linux
# setup_all.bat --skip-db  # Windows
